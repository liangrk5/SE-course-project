...
本产品要实现的功能如下用例图：对于软件用户来说，他可以实用的功能有：修改密码、学习新单词、复习旧单词、将学习成果分享至社交软件、查看自己学习排名。对于软件管理员来说，他需要维护单词词库以及用户数据词库并且完成用户修改密码的请求。

![4](img/用例图.png)

类图如下：我们的项目大致可以分为三个类：User、Administrator、Dataset，下面分别说明。

- User类
  - 成员变量包括用户名、用户ID、密码、用户的单词词库及其大小，是一些用户的基本信息，用户ID唯一标识用户。
  - 成员函数包括：修改用户密码、向数据库申请单词、分享学习成果的函数。返回值都是bool，体现操作是否成功。

- Administrator类
  - 成员变量有：管理员ID和管理员密码，ID唯一标识一个管理员身份。
  - 成员函数有：修改单词数据库和用户数据库的操作。

- Database类
  - 成员变量记录了数据库中单词的总个数和用户的总个数。
  - 成员函数包含了对数据库中单词以及用户元组的修改操作，以及读取一部分单词返回给User的功能。

 ![5](img/类图.png)
 
 下面给一实例，如对象图所示：
 
一个用户名为李四，账号ID为222222的人，他目前申请了4500个单词到他的单词库word_list中。

当前总数据库中有1000000个单词和5000个用户。

当前ID为123456的管理员负责对当前数据库的维护的操作。
